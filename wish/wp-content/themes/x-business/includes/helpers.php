...
if ( ! function_exists( 'x_business_get_slider_details' ) ) :
	 ...
	 */
	function x_business_get_slider_details() {

		$output = array();

		$slider_number = 5;

		$slider_excerpt_length  = x_business_get_option('slider_excerpt_length');

		$page_ids = array();

		for ( $i = 1; $i <= $slider_number ; $i++ ) {
			$page_id = x_business_get_option( "slider_page_$i" );
			if ( absint( $page_id ) > 0 ) {
		...
		if ( ! empty( $posts ) ) {
			...
			foreach ( $posts as $post ) {
				if ( has_post_thumbnail( $post->ID ) ) {
					...
					$output[ $p_count ]['excerpt']   = x_business_get_the_excerpt( absint($slider_excerpt_length), $post );
...
if ( ! function_exists( 'x_business_get_the_excerpt' ) ) :
	 ...
	 */
	function x_business_get_the_excerpt( $length = 0, $post_object = null ) {
		global $post;
...
if ( ! function_exists( 'x_business_fonts_url' ) ) :
	 ...
	 */
	function x_business_fonts_url() {
		$fonts_url = '';
		$fonts     = array();
		$subsets   = 'latin,latin-ext';

		/* translators: If there are characters in your language that are not supported by Poppins, translate this to 'off'. Do not translate into your own language. */
		if ( 'off' !== _x( 'on', 'Poppins font: on or off', 'x-business' ) ) {
			$fonts[] = 'Poppins:200,200i,300,300i,400,400i,500,500i,600,600i,700,700i,800,800i';
		}
		/* translators: If there are characters in your language that are not supported by Open Sans, translate this to 'off'. Do not translate into your own language. */
		if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'x-business' ) ) {
		...
		if ( 'off' !== _x( 'on', 'Raleway font: on or off', 'x-business' ) ) {
